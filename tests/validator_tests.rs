use pdmt::models::todo::{Todo, TodoList, TodoStatus, TodoPriority, TodoQualityConfig};
...
fn test_validator_with_custom_config() {
    ...
    assert!(result.issues.iter().any(|i| i.category == IssueCategory::Actionability));
    assert!(result.issues.iter().any(|i| i.category == IssueCategory::Completeness));
    assert!(result.issues.iter().any(|i| i.category == IssueCategory::TimeEstimate));
}
...
fn test_validator_dependency_checks() {
    ...
    assert!(result.issues.iter().any(|i| 
        i.category == IssueCategory::Dependencies && 
...
fn test_validator_generic_language_detection() {
    ...
    let generic_issues: Vec<_> = result.issues.iter()
