use pdmt::{TemplateEngine, TemplateDefinition};
...
fn test_template_parameter_management() {
    ...
    template.set_parameter("string_param".to_string(), "value").unwrap();
    template.set_parameter("number_param".to_string(), 42).unwrap();
    template.set_parameter("bool_param".to_string(), true).unwrap();
    template.set_parameter("array_param".to_string(), vec![1, 2, 3]).unwrap();
...
fn test_template_determinism_checks() {
    ...
    template.set_parameter("temperature".to_string(), 0.8).unwrap();
    assert!(!template.is_deterministic());
    
    // Set temperature to 0
    template.set_parameter("temperature".to_string(), 0.0).unwrap();
    assert!(template.is_deterministic());
    
    // Deterministic provider always deterministic
    template.metadata.provider = "deterministic".to_string();
    template.set_parameter("temperature".to_string(), 1.0).unwrap();
    assert!(template.is_deterministic());
}
...
fn test_template_validation_rules() {
    ...
    template.metadata.provider = "openai".to_string();
    template.set_parameter("temperature".to_string(), 0.5).unwrap();
...
fn test_template_quality_enforcement() {
    use pdmt::template::definition::{QualityEnforcement, QualityMode, PmatConfig};
