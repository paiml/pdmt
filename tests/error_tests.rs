use pdmt::error::{Error, TemplateError, ValidationError, Result};
...
fn test_quality_error_creation() {
    ...
    assert_eq!(violation_with_location.location, Some("new_file.rs:20".to_string()));
    
    let violation_with_suggestion = violation.clone().with_suggestion("Refactor the code");
    assert_eq!(violation_with_suggestion.suggestion, Some("Refactor the code".to_string()));
...
fn test_todo_validation_error() {
    ...
    let err = TodoValidationError::CircularDependency {
        cycle: vec!["task1".to_string(), "task2".to_string(), "task1".to_string()],
    };
